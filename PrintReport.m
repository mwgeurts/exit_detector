...
handles.tcsplot = ImageViewer('axis', handles.axes4, 'tcsview', data.tcsview, ...
    'background', data.referenceImage, 'overlay', data.referenceDose, ...
    'alpha', sscanf(get(data.alpha, 'String'), '%f%%')/100, ...
    'slice', round(get(data.dose_slider, 'Value')), ...
    'structures', data.referenceImage.structures, ...
    'structuresonoff', get(data.dvh_table, 'Data'), 'cbar', 'on', ...
    'zoom', 'off', 'pixelval', 'off');
